...
const filterStringsWithVowels = (arr) => {
  ...
  return arr.filter(el=>el.match(regex));
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  return arr.filter(el=>forbiddenValues.includes(el)===false);
};
...
const getCharactersWithoutChildren = (arr) => {
  return arr.filter(el=>el.children===undefined || el.children.length<1);
};
...
const evenOddNumericValues = (arr) => {
  let filterArr = arr.filter(el=>Number.isInteger(el)===true);
  return filterArr.map(el=>el%2===0? 'even':'odd');
};
